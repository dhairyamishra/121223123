...
router.get('/',function(req,res,next) {
    ...
    log.silly(JSON.stringify({msg:'get book browse'}));
...
router.post('/browseBy', function(req,res,next) {
    browseBy(req.body.type, function(err, HTML) {
        if (err) return next(err);
...
function getPage(callback) {
    browseBy('info.title', function(err, resultHTML) {
        if (err) return callback(err, undefined);

        callback(undefined,Template(resultHTML));
    });
}

function Template(resultHTML) {
    var body = `<nav class="navbar navbar-dark bg-primary">
                ...
                <div class="container" id="main">
                    <div class="row">
                        <div class="col-lg">
                            <h3 class="pt-2 text-center">Browse Books</h3>
                            <br>
                            <div class="text-light pt-2 px-2">
                                <div class="container">
                                    <div class="row justify-content-center">

                                        <div class="col-xs mb-2 pl-0 pr-4">
                                            <h5 class="pt-2 text-dark">Sort by:</h5>
                                        </div>
                                    ...
                                    <div class="row justify-content-center">
                                        <div class="w-75" id="browseResults">
                                            ${resultHTML}
                                        </div>
                                    </div>
...
function browseBy(type, callback) {
    ...
    mon.select(collection, attributes, query, sort, function(err, res) {
        ...
        callback(undefined, genHTML(res));
    });
}

function genHTML(booklist) {
    var html = '';
